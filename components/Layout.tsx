...
const query = useGQLQuery(
  ...
  {},
  {
    ...
    //  console.log("main user query error ", error);
     ...
     globalCtx.updateValue({type:'ERROR',payload:error?.response?.message})
     }
      setValidating(false)
    },
  }
...
{ return <div className="w-full min-h-screen flex-center">loading.....</div>;}

if(shouldLogin() ){ return <Login/>}
...
return (
  <div className="w-full min-h-screen h-full flex flex-col justify-between dark-styles">
    <ViewerContext.Provider value={{value:{viewer,token},updateValue:setValue}}>
